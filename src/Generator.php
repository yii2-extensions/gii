...
abstract class Generator extends Model
{
    ...
    {
        return Yii::$app->getRuntimePath() . '/gii-' . Yii::getVersion() . '/' . str_replace('\\', '-', static::class) . '.json';
    }
     ...
     */
    public function validateClass($attribute, $params): void
    {
        $class = $this->$attribute;
        try {
            if (class_exists($class)) {
                if (isset($params['extends'])) {
                    if (ltrim($class, '\\') !== ltrim((string) $params['extends'], '\\') && !is_subclass_of($class, $params['extends'])) {
    ...
    {
        $class = ltrim((string) $this->$attribute, '\\');
     ...
     */
    public function validateMessageCategory(): void
    {
        if ($this->enableI18N) {
    ...
    public function generateString($string = '', $placeholders = [])
    {
        ...
        if ($this->enableI18N) {
            ...
            if (!empty($placeholders)) {
                $phKeys = array_map(fn($word) => '{' . $word . '}', array_keys($placeholders));
