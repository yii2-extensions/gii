...
class CodeFile extends BaseObject
{
     ...
     */
    public function __construct($path, public $content, $config = [])
    {
    ...
    {
        if (str_starts_with($this->path, (string) Yii::$app->basePath)) {
            return substr($this->path, strlen((string) Yii::$app->basePath) + 1);
        }
