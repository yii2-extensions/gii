...
{
    final public const RELATIONS_NONE = 'none';
    final public const RELATIONS_ALL = 'all';
    final public const RELATIONS_ALL_INVERSE = 'all-inverse';
    final public const JUNCTION_RELATION_VIA_TABLE = 'table';
    final public const JUNCTION_RELATION_VIA_MODEL = 'model';
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [
                ['ns', 'queryNs'],
                'filter',
                'filter' => static fn($value) => $value === null ? null : trim((string) $value, ' \\'),
            ],
     ...
     * @return \Closure[]
     *
     * @psalm-return array{tableName?: \Closure():array<string>}
     */
    public function autoCompleteData(): array
    {
        ...
        if ($db !== null) {
            return [
                'tableName' => fn() => $db->getSchema()->getTableNames(),
            ];
        }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', (string) $this->ns)) . '/' . $modelClassName . '.php',
            ...
            if ($queryClassName) {
                ...
                $files[] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', (string) $this->queryNs)) . '/' . $queryClassName . '.php',
    ...
    protected function generateProperties($table)
    {
        ...
        foreach ($table->columns as $column) {
            $type = match ($column->type) {
                Schema::TYPE_SMALLINT, Schema::TYPE_INTEGER, Schema::TYPE_BIGINT, Schema::TYPE_TINYINT => 'int',
                Schema::TYPE_BOOLEAN => 'bool',
                Schema::TYPE_FLOAT, Schema::TYPE_DOUBLE, Schema::TYPE_DECIMAL, Schema::TYPE_MONEY => 'float',
                Schema::TYPE_DATE, Schema::TYPE_TIME, Schema::TYPE_DATETIME, Schema::TYPE_TIMESTAMP, Schema::TYPE_JSON => 'string',
        ...
        foreach ($relations as $name => $relation) {
            ...
            if ($generateQuery) {
                ...
                if (class_exists($queryClassRealName, true) && is_subclass_of($queryClassRealName, '\yii\db\BaseActiveRecord')) {
                    ...
                    if (str_starts_with($activeQueryClass, (string) $this->ns)) {
     ...
     */
    private function generateManyManyRelations(TableSchema $table, $fks, $relations)
    {
        $db = $this->getDbConnection();
        ...
        foreach ($fks as $pair) {
            ...
            if ($this->generateJunctionRelationMode === self::JUNCTION_RELATION_VIA_TABLE) {
                ...
                $relations[$table0Schema->fullName][$relationName] = [
                    ...
                    . lcfirst((string) $foreignRelationName) . "');",
                    $className1,
                    true,
                ];
            }
            ...
            foreach ($db->schema->getTableSchemas($schemaName) as $table) {
                ...
                foreach ($table->foreignKeys as $refs) {
                    ...
                    $relations[$table->fullName][$leftRelationName][0] =
                        rtrim((string) $relations[$table->fullName][$leftRelationName][0], ';')
                        . "->inverseOf('" . lcfirst($rightRelationName) . "');";
                    $relations[$refTableSchema->fullName][$rightRelationName][0] =
                        rtrim((string) $relations[$refTableSchema->fullName][$rightRelationName][0], ';')
    ...
    {
        if ((is_countable($table->foreignKeys) ? count($table->foreignKeys) : 0) < 2) {
            return false;
        }
        $uniqueKeys = [$table->primaryKey];
        try {
        ...
        for ($i = 0; $i < $foreignKeysCount; $i++) {
            ...
            for ($j = $i + 1; $j < $foreignKeysCount; $j++) {
                ...
                $fks = [...array_keys($firstColumns), ...array_keys($secondColumns)];
                foreach ($uniqueKeys as $uniqueKey) {
     ...
     */
    public function validateNamespace($attribute): void
    {
    ...
    {
        if (str_contains($this->tableName, '*') && substr_compare($this->tableName, '*', -1, 1)) {
    ...
    protected function generateQueryClassName($modelClassName)
    {
        ...
        if (empty($queryClassName) || str_contains($this->tableName, '*')) {
